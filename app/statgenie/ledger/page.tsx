...
function MoneyInput({ label, value, onChange, allowNegative }: { label: string; value: string; onChange: (v: string) => void; allowNegative?: boolean }) {
  return (
    <div className="flex items-center gap-4">
      ...
      <div className="flex items-center border border-[#e9e9e7] dark:border-[#3f3f3f] rounded-lg bg-[#f7f6f3] dark:bg-[#2f2f2f] overflow-hidden max-w-72 w-full focus-within:border-amber-400 transition-colors">
...
export default function GenieLedgerPage() {
  const router = useRouter();
  const { entries, upsert, remove } = useGenieLedger();
      ...
      <div className="px-6 py-6">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            <button onClick={() => router.push("/statgenie")}
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={handleSave} disabled={saveStatus === "saving"}
              className="flex items-center gap-1.5 px-3 py-1.5 text-sm rounded-lg bg-amber-400 text-amber-900 font-bold hover:bg-amber-500 disabled:opacity-50 transition-colors">
              <Save size={13} /> 저장
            </button>
          </div>
        </div>
                ...
                <button key={yr}
                  ...
                  className={cn(
                    ...
                    currentYear === yr
                      ...
                      : "bg-white dark:bg-[#252525] text-[#9b9a97] border border-[#e9e9e7] dark:border-[#2f2f2f] hover:border-amber-400"
                  )}>
                  {yr}년
                </button>
              ))}
                ...
                {Array.from({ length: 12 }, (_, i) => i + 1).map((mo) => {
                  ...
                  return (
                    <div key={mo} className={cn("group flex items-center transition-colors",
                      active ? "bg-amber-50 dark:bg-amber-950/20" : "hover:bg-[#f7f6f3] dark:hover:bg-[#2f2f2f]")}>
                      <button onClick={() => setMonth(`${currentYear}-${String(mo).padStart(2, "0")}`)}
                        className={cn("flex-1 flex items-center justify-between px-4 py-2 text-sm",
                          active ? "text-amber-600 dark:text-amber-400 font-semibold" : "text-[#37352f] dark:text-[#e6e6e4]")}>
                        <span>{mo}월</span>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-3 mb-6">
              <button onClick={() => setMonth(shiftYM(month, -1))}
                className="w-9 h-9 flex items-center justify-center rounded-lg border border-[#e9e9e7] dark:border-[#3f3f3f] bg-white dark:bg-[#252525] text-[#9b9a97] hover:text-amber-500 hover:border-amber-400 transition-colors">
                <ChevronLeft size={16} />
              </button>
            ...
            {/* 입력 카드 */}
            <div className="bg-white dark:bg-[#252525] rounded-2xl border border-[#e9e9e7] dark:border-[#2f2f2f] border-t-4 border-t-amber-400 shadow-sm overflow-hidden mb-4">
              <div className="flex items-center gap-3 px-5 py-4 border-b border-[#e9e9e7] dark:border-[#2f2f2f]">
                <div className="w-7 h-7 rounded-full bg-amber-400 text-amber-900 text-xs font-bold flex items-center justify-center">1</div>
              ...
              <div className="px-5 py-4 flex flex-col gap-3">
                ...
                <ReadonlyMoney label="영업이익" value={operatingProfit} accent={operatingProfit < 0 ? "text-red-500" : "text-amber-600 dark:text-amber-400"} />
              ...
              <div className={`flex items-center justify-between px-5 py-3 ${profit < 0 ? "bg-red-50 dark:bg-red-950/30 border-l-4 border-red-500" : "bg-amber-50 dark:bg-amber-950/20 border-l-4 border-amber-400"}`}>
                <div>
                ...
                <span className={`text-lg font-extrabold ${profit < 0 ? "text-red-500" : "text-amber-600 dark:text-amber-400"}`}>{fmt(profit)}</span>
              </div>
            </div>
