...
export default function StatGeniePage() {
  ...
  useEffect(() => {
    if (!isSupabaseConfigured || !supabase) return;
    const ch = supabase!
      .channel("genie_cost_entries")
      .on("postgres_changes", { event: "*", schema: "public", table: "genie_cost_entries" }, loadEntries)
      .subscribe();
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    const { error } = await supabase.from("genie_cost_entries").insert(item);
    setSaving(false);
  ...
  const handleDelete = async (id: string) => {
    if (!isSupabaseConfigured || !supabase) return;
    await supabase.from("genie_cost_entries").delete().eq("id", id);
    setDeleteId(null);
    loadEntries();
  };
  ...
  const handleSaveEdit = async () => {
    ...
    await supabase.from("genie_cost_entries").update({
    ...
    <div className="flex-1 overflow-y-auto bg-[#f5f5f7] dark:bg-[#191919]">
      <div className="max-w-4xl mx-auto px-6 py-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-5">
          <div className="bg-white dark:bg-[#252525] rounded-2xl border border-[#e9e9e7] dark:border-[#2f2f2f] shadow-sm p-5 border-l-4 border-l-amber-400">
            <p className="text-xs font-bold text-[#9b9a97] uppercase tracking-wide mb-1">총 투자비용</p>
            <p className="text-2xl font-extrabold text-amber-500">{fmt(totalCost)}</p>
        ...
        {/* 비용 내역 추가 */}
        <div className="bg-white dark:bg-[#252525] rounded-2xl border border-[#e9e9e7] dark:border-[#2f2f2f] border-t-[3px] border-t-amber-400 shadow-sm overflow-hidden mb-6">
            ...
            <div className="flex flex-wrap gap-3 items-end">
              ...
              <div className="flex flex-col gap-1">
                ...
                <div className="flex items-center gap-1.5">
                  <select value={year} onChange={e => setYear(Number(e.target.value))}
                    className="px-2 py-2 text-sm border border-[#e9e9e7] dark:border-[#3f3f3f] rounded-lg bg-[#f7f6f3] dark:bg-[#2f2f2f] text-[#37352f] dark:text-[#e6e6e4] outline-none focus:border-amber-400">
              ...
              <div className="flex flex-col gap-1 flex-1 min-w-[180px]">
                ...
                <input value={desc} onChange={e => setDesc(e.target.value)} placeholder="예) 서버 호스팅, 광고비, 개발 도구…" required
                  className="px-3 py-2 text-sm border border-[#e9e9e7] dark:border-[#3f3f3f] rounded-lg bg-[#f7f6f3] dark:bg-[#2f2f2f] text-[#37352f] dark:text-[#e6e6e4] outline-none focus:border-amber-400 transition-colors" />
              </div>
              {/* 금액 */}
              ...
              <button type="submit" disabled={saving}
                className="flex items-center gap-1.5 px-4 py-2 bg-gradient-to-r from-yellow-400 to-amber-500 text-amber-900 font-bold rounded-lg text-sm hover:opacity-90 disabled:opacity-60 transition-all whitespace-nowrap">
            ...
            <div className="divide-y divide-[#e9e9e7] dark:divide-[#2f2f2f]">
              {groupKeys.map(key => {
                ...
                return (
                  <div key={key}>
                    {/* 월 헤더 */}
                    <div className="flex items-center justify-between px-5 py-2.5 bg-amber-50 dark:bg-amber-950/20 border-b border-amber-100 dark:border-amber-900/30">
                      <span className="text-sm font-bold text-amber-700 dark:text-amber-400">{gy}년 {parseInt(gm)}월</span>
                      <span className="text-sm font-bold text-amber-700 dark:text-amber-400">{fmt(groupTotal)}</span>
                    </div>
                    {/* 항목 */}
                            ...
                            <td className="px-5 py-3">
                              <div className="flex items-center gap-1">
                                <button onClick={() => openEdit(item)}
                                  className="p-1.5 text-[#9b9a97] hover:text-amber-500 hover:bg-amber-50 dark:hover:bg-amber-950/30 rounded-md transition-colors">
                                  <Pencil size={13} />
                                </button>
      ...
      {editItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={() => setEditItem(null)}>
          <div className="bg-white dark:bg-[#252525] rounded-2xl border border-[#e9e9e7] dark:border-[#3f3f3f] border-t-4 border-t-amber-400 shadow-xl p-6 w-full max-w-md mx-4"
            ...
            <div className="flex flex-col gap-4">
              ...
              <div>
                ...
                <input value={editDesc} onChange={e => setEditDesc(e.target.value)}
                  className="w-full px-3 py-2 text-sm border border-[#e9e9e7] dark:border-[#3f3f3f] rounded-lg bg-[#f7f6f3] dark:bg-[#2f2f2f] text-[#37352f] dark:text-[#e6e6e4] outline-none focus:border-amber-400 transition-colors" />
              </div>
              <div>
            ...
            <div className="flex gap-2 mt-5">
              ...
              <button onClick={handleSaveEdit}
                className="flex-1 py-2 rounded-lg bg-gradient-to-r from-yellow-400 to-amber-500 text-amber-900 text-sm font-bold hover:opacity-90 transition-all">
                저장
              </button>
            </div>
          </div>
